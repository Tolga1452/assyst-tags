...
async function experimentRollout(command, id) {
    ...
    else if (rolloutType === 2) description = `<:ticket:1100811774229495858> This feature is very slowly rolling out to all ${experimentType === 0 ? 'servers' : experimentType === 1 ? 'users' : 'servers and users'} from old to new ones, this process may take a few months.`;
    else if (rolloutType === 3) description = `🥳 This feature is a part of **Birthday Activities**. Releases ||**<t:${timestamp}:R>**||.`;
    else if (rolloutType === 4) description = `<:DEH:1098207702682980404> This feature is currently in beta/alpha testing.`;
